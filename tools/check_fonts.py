...
FONT_SMALL = ImageFont.load("assets/fonts/pil/7x14B.pil")
...
# Load bitmap fonts
FONT_SMALLEST = ImageFont.load("assets/fonts/pil/4x6.pil")
...
def print_glyph_pixels(font):
    for c in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
        ...
        if bbox:
            ...
            for y in range(h):
                line = "".join(
                    ["#" if cropped.getpixel((x, y)) else "." for x in range(w)]
                )
                print(line)
        print()
